...
#畫布截圖比對
def menu_compare(sample, case,num):
    ...
    print("menu"+str(num)+"所有點的數量: ", allpoint, " / 有誤差的點數量: ", errorCount, " / 誤差率: ",float(errorCount/allpoint))
    if errorCount / allpoint < 0.01:  #預估錯誤比例不可多於1%，超過即回傳False
...
def canvas_compare(sample, case,num):
    ...
    # print("全部的點: ", allpoint)
    # print("誤差值: ", errorCount)
    print("canvas"+str(num)+"所有點的數量: ", allpoint, " / 有誤差的點數量: ", errorCount, " / 誤差率: ",float(errorCount/allpoint))
    if errorCount / allpoint < 0.01:  #預估錯誤比例不可多於1%，超過即回傳False
