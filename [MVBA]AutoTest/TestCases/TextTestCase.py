...
def testCase1(self):
    try:
        ...
        LogPack.AddLogRow(sheetName, "點擊Tool bar上的Text icon", "執行結果", step_img)
        Text.SelectTextBtn(self)
        Text.SelectTextBtn(self)
        Canvas.Tap(self, 900, 250)
        Canvas.Tap(self, 900, 250)
        Screenshot.screenshotTextEditor(self, step_img)
        LogPack.AddLogRow(sheetName, "點擊畫布建立文字編輯框", "執行結果", step_img)
        ...
        LogPack.AddLogRow(sheetName, "更改Font", "執行結果", step_img)
        ...
        LogPack.AddLogRow(sheetName, "輸入文字'testcaseone'", "執行結果", step_img)
        Screenshot.screenshotTextEditor(
            self, Case1_Editor_img)
        LogPack.AddLogRow(sheetName, "擷取文字編輯框上的工具列", "執行結果", Case1_Editor_img)
        ...
        LogPack.AddLogRow(sheetName, "關閉文字編輯框", "執行結果", step_img)
        Screenshot.screenshotCanvas(
            self, Case1_Canvas_img)
        LogPack.AddLogRow(sheetName, "擷取畫布內容", "執行結果", Case1_Canvas_img)
        if not Compare.isImgEqual(Case1_Editor_img, "./TestCases/Samples/Text/Case1_Editor.png"):
            LogPack.AddLogRow(sheetName, "比對編輯框內容", "Fail!")
        else:
            LogPack.AddLogRow(sheetName, "比對編輯框內容", "Pass!")
        if not Compare.isImgEqual(Case1_Canvas_img, "./TestCases/Samples/Text/Case1_Canvas.png"):
            LogPack.AddLogRow(sheetName, "比對畫布內容", "Fail!")
        else:
            LogPack.AddLogRow(sheetName, "比對畫布內容", "Pass!")
...
def testCase2(self):
    try:
        ...
        LogPack.AddLogRow(sheetName, "套用Italic", "執行結果", step_img)
        Text.Italic(self)
        ...
        LogPack.AddLogRow(sheetName, "點擊文字顏色icon，並點擊”ADVANCED", "執行結果", step_img)
        ...
        LogPack.AddLogRow(sheetName, "點擊底色icon，並點擊“STANDARD", "執行結果", step_img)
        Text.ChangeColorBg(self, ColorNum=9)
        Text.TypeText(self, "testcasetwo")
        editText_id = ElementParam._Id_editText
        editText = self.driver.find_element_by_id(editText_id)
        editText.screenshot(step_img)
        LogPack.AddLogRow(sheetName, "輸入文字'testcasetwo'", "執行結果", step_img)
        Screenshot.screenshotTextEditor(
            self, Case2_Editor_img)
        LogPack.AddLogRow(sheetName, "擷取文字編輯框上的工具列", "執行結果", Case2_Editor_img)
        buttonMenuClose_id = ElementParam._Id_buttonMenuClose
        buttonMenuClose = self.driver.find_element_by_id(buttonMenuClose_id)
        buttonMenuClose.screenshot(step_img)
        Text.CloseEditor(self)
        ...
        LogPack.AddLogRow(sheetName, "擷取畫布內容", "執行結果", Case2_Canvas_img)
...
def testCase3(self):
    try:
        ...
        LogPack.AddLogRow(sheetName, "套用UnderLine", "執行結果", step_img)
        Text.UnderLine(self)
        FontColor_id = ElementParam._Id_checkbuttonColor
        FontColor = self.driver.find_element_by_id(FontColor_id)
        FontColor.screenshot(step_img)
        LogPack.AddLogRow(sheetName, "點擊文字顏色icon，並點擊”STANDARD", "執行結果", step_img)
        Text.ChangeColor(self, ColorNum=6)
        FontColorBg_id = ElementParam._Id_checkbuttonColorBg
        FontColorBg = self.driver.find_element_by_id(FontColorBg_id)
        FontColorBg.screenshot(step_img)
        LogPack.AddLogRow(sheetName, "點擊底色icon，並點擊“ADVANCED", "執行結果", step_img)
        Text.ChangeColorBg(self, "Advanced", R=110, G=200, B=90)
        Text.TypeText(self, "testcasethree")
        editText_id = ElementParam._Id_editText
        editText = self.driver.find_element_by_id(editText_id)
        editText.screenshot(step_img)
        LogPack.AddLogRow(sheetName, "輸入文字'testcasethree'", "執行結果", step_img)
        Screenshot.screenshotTextEditor(
            self, Case3_Editor_img)
        LogPack.AddLogRow(sheetName, "擷取文字編輯框上的工具列", "執行結果", Case3_Editor_img)
        buttonMenuClose_id = ElementParam._Id_buttonMenuClose
        buttonMenuClose = self.driver.find_element_by_id(buttonMenuClose_id)
        buttonMenuClose.screenshot(step_img)
        Text.CloseEditor(self)
        ...
        LogPack.AddLogRow(sheetName, "擷取畫布內容", "執行結果", Case3_Canvas_img)
